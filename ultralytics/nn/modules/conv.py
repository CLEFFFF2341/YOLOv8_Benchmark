...
class CA(nn.Module):
    """Coordinate Attention module for position-aware feature recalibration."""
    ...
    def __init__(self, c1: int, reduction: int = 32):
        ...
        Args:
            c1 (int): Number of input channels.
            reduction (int): Channel reduction ratio for the shared transform.
        """
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply coordinate attention to input features."""
        _, _, h, w = x.size()
        x_h = self.pool_h(x)
